...
class URLBulkOpener {
    ...
    initializeElements() {
        ...
        this.logo = document.querySelector('.logo h2');

        // No modal on pages
    }

    attachEventListeners() {
        // Input events (only on pages that have the input)
        if (this.urlInput) {
        ...
        // Logo click to refresh page with smooth fade
        if (this.logo) {
    ...
    updateActionButtons() {
        ...
        if (hasValidUrls) {
            ...
            if (hasSelectedUrls) {
                if (remainingUrls > 0) {
                    this.openSelectedBtn.innerHTML = `<i class="fas fa-external-link-alt"></i> Open Next URL (${remainingUrls} remaining)`;
                } else {
